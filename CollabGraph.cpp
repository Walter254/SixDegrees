 ...
 **   4 DEC 2021: Walter Wagude
 **       Edited the file - completed the rest of the implementation of a 
 **       collabgraph.
 **/
...
void CollabGraph::populate_graph(const vector<Artist>& artists) {
    // string to store the song the artists collaborated in
    string song;
    
    // insert the vertices
    for (size_t i = 0; i < artists.size(); i++){
        insert_vertex(artists[i]);
    }
    
    // insert the edges
    for (size_t i = 0; i < artists.size(); i++){
        for (size_t j = i+1; j < artists.size(); j++){
            song = artists[i].get_collaboration(artists[j]);
            if (song != ""){
                insert_edge(artists[i], artists[j], song);
            }
        }
    }    
}
...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist& artist) const {
    ...
    Vertex *to = graph.at(artist.get_name());
    // traverse through the graph and get the artists' neighbors
    for (size_t i = 0; i < to->neighbors.size(); i++){
        Vertex *nptr = to->neighbors[i].neighbor;
        neighbors.push_back(nptr->artist);
    }
...
stack<Artist> CollabGraph::report_path(const Artist& source,
    ...
    Artist current = dest;
    // if the source is the same as destination, return an empty stack
    if (source == dest){
        return path;
    }
    
    // push the atists on the stack - the ones on the path including the
    // the destination and the source
    while (current != source){
        path.push(current);
        current = get_predecessor(current);
        if (current == source){
            // push the source last so that it is on the top of the stack
            path.push(source);
            // return a non empty stack with atleast two artists
            return path;
        }
        
    }
    // return an empty stack - indication that there is no path between the
    // vertices
    while (!path.empty()){
        path.pop();
    }
